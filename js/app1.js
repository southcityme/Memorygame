...
// 点击事件
function clickCard(element,i) {
	element[i].onclick=clickCarddetail(i);
}
function  clickCarddetail(i){
	return function () {
		if (Oli[i].className=="card"&&flagfinish==1) {
			if (flag==1) {
				Oli[i].className="card open show";
				intervarelea=i;
				intervar=Oli[i].children[0].className;
				flag=2;
			}
			else{
				if (flag==2) {
					intervareleb=i;
					if (intervar==Oli[i].children[0].className) {
						Oli[i].className="card match animated flipInY";
						...
						Oli[i].className="card defaule show animated wobble";
